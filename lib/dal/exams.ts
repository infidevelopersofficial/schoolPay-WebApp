...
export async function createExam(input: z.infer<typeof createExamSchema>) {
  ...
  return withDAL(
    ...
    async () => {
      ...
      const [batch, subject, session, teacher] = await Promise.all([
        ...
        validated.teacherId ? prisma.teacher.findUnique({ where: { id: validated.teacherId, schoolId } }) : Promise.resolve(true),
      ])
      ...
      if (!teacher) throw new Error("Teacher not found or belongs to another school")
