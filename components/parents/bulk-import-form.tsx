...
import { importParentsBatchAction } from "@/app/(dashboard)/dashboard/parents/import/actions";
...
export function BulkImportForm() {
  ...
  const [successResult, setSuccessResult] = useState<{ imported: number, skipped: number, rejectedRows?: any[] } | null>(null);
    ...
    data.forEach((row, index) => {
      ...
      REQUIRED_FIELDS.forEach(field => {
        if (!row[field] || String(row[field]).trim() === "") {
          if (field === "phone") {
             rowErrors.push("Phone is required to set initial login password");
          } else {
      ...
      if (row.email) {
        if (emails.has(row.email.toLowerCase())) {
          rowErrors.push(`Duplicate email within file: ${row.email}`);
        }
      ...
      if (row.phone && !/^\+?\d{10,15}$/.test(row.phone.toString().replace(/\s/g, ''))) {
  ...
  const handleImport = async () => {
    ...
    try {
      for (let i = 0; i < totalChunks; i++) {
        ...
        const res = await importParentsBatchAction(chunk);
        ...
        if ("success" in res && res.success) {
          ...
          if (res.rejectedRows) allRejectedRows = [...allRejectedRows, ...res.rejectedRows];
        }

        setProgress(Math.round(((i + 1) / totalChunks) * 100));
      }

      setSuccessResult({ imported: totalImported, skipped: totalSkipped, rejectedRows: allRejectedRows });
      toast.success("Import processing completed.");
      setParsedData([]);
      setFile(null);
    } catch (err: any) {
      toast.error(err.message || "Failed to import parents. Check your data.");
    } finally {
        ...
        <CardHeader>
          <div className="flex justify-between items-start">
            <div>
              ...
              <CardDescription>Upload a CSV or Excel file to bulk import parents.</CardDescription>
            </div>
        ...
        <CardContent>
          ...
          {file && !isImporting && !successResult && (
            <div className="space-y-6">
                          ...
                          <tr key={idx} className={hasErr ? "bg-red-500/5" : "hover:bg-slate-800/20"}>
                            ...
                            <td className="px-2 py-1">
                              <Input 
                                ...
                                className={`h-8 min-w-[120px] bg-transparent border-transparent hover:border-slate-700 focus:bg-slate-900 ${!row.name ? 'border-red-500/50 bg-red-500/5' : ''}`}
                              />
                            </td>
                            <td className="px-2 py-1">
                              <Input 
                                value={row.email || ""} 
                                onChange={(e) => handleInlineEdit(idx, "email", e.target.value)}
                                className={`h-8 min-w-[180px] bg-transparent border-transparent hover:border-slate-700 focus:bg-slate-900 ${!row.email ? 'border-red-500/50 bg-red-500/5' : ''}`}
                              />
                            </td>
                            <td className="px-2 py-1">
                              <Input 
                                value={row.phone || ""} 
                                onChange={(e) => handleInlineEdit(idx, "phone", e.target.value)}
                                className={`h-8 min-w-[120px] bg-transparent border-transparent hover:border-slate-700 focus:bg-slate-900 ${!row.phone ? 'border-red-500/50 bg-red-500/5' : ''}`}
                              />
                            </td>
                            <td className="px-2 py-1">
                              <Input 
                                value={row.relationship || ""} 
                                onChange={(e) => handleInlineEdit(idx, "relationship", e.target.value)}
                            ...
                            <td className="px-2 py-1">
                              <Input 
                                ...
                                onChange={(e) => handleInlineEdit(idx, "occupation", e.target.value)}
                                className="h-8 min-w-[100px] bg-transparent border-transparent hover:border-slate-700 focus:bg-slate-900"
                              />
                            </td>
                          </tr>
                        )
              ...
              >
                Start Import ({parsedData.length} parents)
              </Button>
            </div>
          )}
          ...
          {isImporting && (
            <div className="py-12 space-y-6 text-center">
              ...
              <div className="space-y-2 max-w-sm mx-auto">
                <h3 className="text-lg font-medium">Importing Parents...</h3>
          ...
          {successResult && (
            <div className="py-8 text-center space-y-4">
              ...
              <h3 className="text-2xl font-bold">Import Processing Complete</h3>
              <div className="flex items-center justify-center space-x-4">
                ...
                <Badge variant="outline" className="text-slate-400 border-slate-700 bg-slate-800/50 px-3 py-1">
                  {successResult.skipped} Skipped/Rejected
                </Badge>
              </div>

              {successResult.rejectedRows && successResult.rejectedRows.length > 0 && (
                <div className="mt-6 text-left max-h-64 overflow-auto border border-slate-800 rounded-md">
                   <table className="w-full text-sm text-left">
                     <thead className="text-xs text-slate-400 bg-slate-900/50">
                       <tr><th className="px-4 py-2">Row Data</th><th className="px-4 py-2">Rejection Reason</th></tr>
                     </thead>
                     <tbody className="divide-y divide-slate-800/50 text-slate-300">
                       {successResult.rejectedRows.map((rr, idx) => (
                         <tr key={idx}>
                           <td className="px-4 py-2">{rr.row?.email || rr.row?.name || "Unknown"}</td>
                           <td className="px-4 py-2 text-red-400">{rr.error}</td>
                         </tr>
                       ))}
                     </tbody>
                   </table>
